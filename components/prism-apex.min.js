(function (Prism) {
	...
	var className = '\\b(?:(?=[a-z_]\\w*\\s*[<\\[])|(?!<keyword>))[A-Z_]\\w*(?:\\s*\\.\\s*[A-Z_]\\w*)*\\b(?:\\s*(?:\\[\\s*\\]|<(?:[^<>]|<(?:[^<>]|<[^<>]*>)*>)*>))*'
	...
	Prism.languages.apex = {
		...
		'class-name': [
			{
				pattern: insertClassName('(\\b(?:class|enum|extends|implements|instanceof|interface|new|trigger\\s+\\w+\\s+on)\\s+)<CLASS-NAME>'),
				lookbehind: true,
				inside: classNameInside
			},
			{
				// cast
				pattern: insertClassName('(\\(\\s*)<CLASS-NAME>(?=\\s*\\)\\s*[\\w(])'),
				lookbehind: true,
				inside: classNameInside
			},
			{
				// variable/parameter declaration and return types
				pattern: insertClassName('<CLASS-NAME>(?=\\s*\\w+\\s*[;=,(){:])'),
				inside: classNameInside
			}
		],
		'trigger': {
