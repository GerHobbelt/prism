(function (Prism) {
	...
	Prism.languages.csharp = Prism.languages.extend('clike', {
		'string': [
			{
				pattern: re('(^|[^$\\\\])<<0>>', [verbatimString]),
				lookbehind: true,
				greedy: true
			},
			{
				pattern: re('(^|[^@$\\\\])<<0>>', [regularString]),
				lookbehind: true,
				greedy: true
			},
			{
		...
		'class-name': [
			...
			},
			{
				...
				pattern: re('\\b<<0>>(?=\\s+(?!<<1>>)<<2>>(?:\\s*[=,;:{)\\]]|\\s+(?:in|when)\\b))', [typeExpression, nonContextualKeywords, name]),
				inside: typeInside
			}
		],
		'keyword': keywords,
	...
	Prism.languages.insertBefore('csharp', 'class-name', {
		...
		'type-expression': {
			...
			pattern: re('(\\b(?:default|typeof|sizeof)\\s*\\(\\s*(?!\\s))(?:[^()\\s]|\\s(?!\\s)|<<0>>)*(?=\\s*\\))', [nestedRound]),
			lookbehind: true,
			alias: 'class-name',
			inside: typeInside
		},
	...
	Prism.languages.insertBefore('csharp', 'class-name', {
		'attribute': {
			...
			pattern: re('((?:^|[^\\s\\w>)?])\\s*\\[\\s*)(?:<<0>>\\s*:\\s*)?<<1>>(?:\\s*,\\s*<<1>>)*(?=\\s*\\])', [attrTarget, attr]),
			lookbehind: true,
			greedy: true,
			inside: {
				'target': {
	...
	Prism.languages.insertBefore('csharp', 'string', {
		'interpolation-string': [
			{
				pattern: re('(^|[^\\\\])(?:\\$@|@\\$)"(?:""|\\\\[^]|\\{\\{|<<0>>|[^\\\\{"])*"', [mInterpolation]),
				lookbehind: true,
				greedy: true,
				inside: createInterpolationInside(mInterpolation, mInterpolationRound),
			},
			{
				pattern: re('(^|[^@\\\\])\\$"(?:\\\\.|\\{\\{|<<0>>|[^\\\\"{])*"', [sInterpolation]),
				lookbehind: true,
				greedy: true,
