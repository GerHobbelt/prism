(function (Prism) {
	...
	var casActions = {
		pattern: RegExp('(^|\\s)(?:action\\s+)?(?:<act>)\\.[a-z]+\\b[^;]+'.replace(/<act>/g, function () { return actionSets; }), 'i'),
		lookbehind: true,
		inside: {
			'keyword': RegExp('(?:<act>)\\.[a-z]+\\b'.replace(/<act>/g, function () { return actionSets; }), 'i'),
			'action': {
				pattern: /(?:action)/i,
				alias: 'keyword'
			},
	...
	Prism.languages.sas = {
		...
		'proc-sql': {
			...
			inside: {
				'sql': {
					pattern: RegExp('^[ \t]*(?:select|alter\\s+table|(?:create|describe|drop)\\s+(?:index|table(?:\\s+constraints)?|view)|create\\s+unique\\s+index|insert\\s+into|update)(?:<str>|[^;"\'])+;'.replace(/<str>/g, function () { return stringPattern; }), 'im'),
					alias: 'language-sql',
		...
		'proc-groovy': {
			...
			inside: {
				'comment': comment,
				'groovy': {
					pattern: RegExp('(^[ \t]*submit(?:\\s+(?:load|parseonly|norun))?)(?:<str>|[^"\'])+?(?=endsubmit;)'.replace(/<str>/g, function () { return stringPattern; }), 'im'),
					lookbehind: true,
		...
		'proc-args': {
			pattern: RegExp('(^proc\\s+\\w+\\s+)(?!\\s)(?:[^;"\']|<str>)+;'.replace(/<str>/g, function () { return stringPattern; }), 'im'),
			lookbehind: true,
			inside: args
		},
